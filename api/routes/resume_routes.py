...
def create_resume_route():
    ...
    try:
        ...
        return jsonify({"error": str(e)}), 400 # SQLAlchemyError might be 500 depending on policy
    except Exception as e:
        return jsonify({"error": "An unexpected error occurred"}), 500
...
def update_resume_sections_order_route(resume_id: UUID):
    ...
    if not isinstance(data, list):
        return jsonify({"error": "Request body must be a list of section order data"}), 400
    try:
...
@resume_routes.route('/<uuid:resume_id>/items/<item_type_str>/<uuid:item_id>', methods=['PUT'])
def update_resume_item_route(resume_id: UUID, item_type_str: str, item_id: UUID):
    ...
    if 'content' not in data or 'data' not in data['content']:
         return jsonify({"error": "Request body must contain 'content' with a 'data' object."}), 400

    try:
        updated_item_detail = resume_controller.update_resume_item(resume_id, item_type_str, item_id, data)
...
@resume_routes.route('/<uuid:resume_id>/items/<item_type_str>/<uuid:item_id>', methods=['DELETE'])
def remove_item_from_resume_route(resume_id: UUID, item_type_str: str, item_id: UUID):
    """Remove an item from a resume."""
    try:
        result = resume_controller.remove_item_from_resume(resume_id, item_type_str, item_id)
        return jsonify(result), 200
    except ValueError as e: # Handles 404 or validation errors
        return jsonify({"error": str(e)}), 400 if "not found" not in str(e).lower() else 404
    except SQLAlchemyError as e:
