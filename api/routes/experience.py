...
def get_all_experiences_for_user(user_id: UUID):
    try:
        ...
        experiences = ctrl.get_all_experiences_for_user(
...
def get_experience(experience_id: UUID):
    try:
        include_bullets = request.args.get("include_bullets", "false").lower() == "true"
        include_skills = request.args.get("include_skills", "false").lower() == "true"
        experience = ctrl.get_experience(
            experience_id,
...
@experience_routes.route(
    "/<uuid:experience_id>/skills/<uuid:skill_id>", methods=["POST"]
)
...
@experience_routes.route(
    "/<uuid:experience_id>/skills/<uuid:skill_id>", methods=["DELETE"]
)
