...
def serialize_project(
    ...
    data = {
        ...
        "title": get_resolved_field(
            project_instance, overrides, "title", project_instance.title
        ),
        "role": get_resolved_field(
            project_instance, overrides, "role", project_instance.role
        ),
        "desc_long": get_resolved_field(
            project_instance, overrides, "desc_long", project_instance.desc_long
        ),
    }

    raw_start_date = get_resolved_field(
        project_instance, overrides, "start", project_instance.start
    )
    ...
    raw_end_date = get_resolved_field(
        project_instance, overrides, "end", project_instance.end
    )
    ...
    if not is_resume_context:
        ...
        if include_skills_if_no_override:
            data["skills"] = [
                ...
                for s in project_instance.skills
            ]
...
def create_project(user_id: UUID, data: dict):
    ...
    try:
            ...
            for idx, bullet_content in enumerate(data["bullets"]):
                db.session.add(
                    BulletPoint(
                        ...
                        parent_type=ParentType.project,
                        order_index=idx,
        ...
        return serialize_project(
            project
        )  # Ensure this serializes bullets as an ordered list
...
def update_project(project_id: UUID, data: dict):
    ...
    try:
        ...
        if "end_date" in data:
            project.end = data["end_date"]
        if "desc_long" in data:
            project.desc_long = data["desc_long"]
        # if 'description' in data: project.description_field = data['description']

        if (
            "bullets" in data
        ):  # If 'bullets' key is present, we replace all existing bullets
            if not isinstance(data["bullets"], list) or not all(
                isinstance(b, str) for b in data["bullets"]
            ):
                raise ValueError(
                    "If 'bullets' is provided for update, it must be a list of strings."
                )

            # Delete existing bullets
            BulletPoint.query.filter_by(
                parent_id=project.id, parent_type=ParentType.project.value
            ).delete()
            # Add new bullets
            for idx, bullet_content in enumerate(data["bullets"]):
                if bullet_content:  # Add only non-empty bullets
                    db.session.add(
                        BulletPoint(
                            parent_id=project.id,
                            parent_type=ParentType.project.value,
                            order_index=idx,
                            content=bullet_content,
                        )
                    )
        ...
        db.session.commit()
        updated_project = Project.query.options(
            joinedload(Project.bullets), joinedload(Project.skills)
        ).get(project_id)
        return serialize_project(
            updated_project,
