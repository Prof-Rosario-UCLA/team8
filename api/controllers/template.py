...
def serialize_template(template: Template):
    ...
    return {"id": str(template.id), "name": template.name, "uri": template.uri}


def create_template(data):
    """Creates a new template."""
    try:
        template = Template(name=data.get("name"), uri=data["uri"])
...
def delete_template(template_id):
    ...
    try:
        ...
        if resumes:
            return {
                "message": "Template cannot be deleted because it is in use on a resume"
            }
