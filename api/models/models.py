...
class Resume(Base):
    ...
    items = relationship(
        "ResumeItem", cascade="all, delete-orphan", order_by="ResumeItem.order_index"
    )
...
class ResumeSection(Base):
    ...
    section_type = Column(
        Enum(ResumeItemType, name="rs_section_item_type"), primary_key=True
    )  # e.g., experience, project
    title = Column(
        String, nullable=False
    )  # User-defined title, e.g., "Work Experience"
    ...
    __table_args__ = (
        Index(
            "idx_resume_section_order", "resume_id", "order_index", unique=True
        ),  # Order index should be unique per resume
    )
...
class ResumeItem(Base):
    ...
    )  # ID of the global item (Education, Experience, etc.)
    ...
    overridden_skills = relationship(
        "Skill", secondary=lambda: resume_item_skills, backref="resume_items"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_resume_items_order_in_section",
            "resume_id",
            "item_type",
            "order_index",
            unique=True,
...
resume_item_skills = Table(
    ...
    Column(
        "item_type",
        Enum(ResumeItemType, name="resume_item_skill_item_type"),
        nullable=False,
    ),  # Name must be unique for Enum type in DB
    Column("item_id", UUID(as_uuid=True), nullable=False),
    Column(
        "skill_id", ForeignKey("skills.id", ondelete="CASCADE"), primary_key=True
    ),  # skill_id is part of PK
    ForeignKeyConstraint(
    ...
    PrimaryKeyConstraint(
        "resume_id", "item_type", "item_id", "skill_id"
...
class ResumeBullet(Base):
    ...
    item_type = Column(
        Enum(ResumeItemType, name="resume_bullet_item_type"), primary_key=True
    )
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["resume_id", "item_type", "item_id"],
            [
                "resume_items.resume_id",
                "resume_items.item_type",
                "resume_items.item_id",
            ],
            ondelete="CASCADE",
        ),
    )
